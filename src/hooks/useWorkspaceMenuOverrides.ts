...
export function useWorkspaceMenuOverridesAdmin(workspaceId?: string) {
  ...
  const setBulk = useMutation({
    mutationFn: async (
      ...
      const toDelete = items.filter((i) => i.visibility === "visible");
      const toUpsert = items.filter((i) => i.visibility !== "visible");
      ...
      for (const chunk of toDelete) {
        const { error } = await supabase
          ...
          .delete()
          .eq("workspace_id", workspaceId)
          .eq("item_type", chunk.itemType)
          .eq("item_key", chunk.itemKey);
        if (error) throw error;
      }
