    ...
    mutationFn: async (input: CreateOrderNoteInput) => {
      ...
      const { data: orderNote, error: orderError } = await supabase
        ...
        .insert({
          ...
          currency: "EUR",
          billing_address: (input.billing_address ? JSON.parse(JSON.stringify(input.billing_address)) : {}) as any,
          shipping_address: (input.shipping_address ? JSON.parse(JSON.stringify(input.shipping_address)) : {}) as any,
