...
export function useCompanies(options?: { archiveState?: CompaniesArchiveState }) {
  ...
  const updateCompany = useMutation({
    mutationFn: async ({ id, ...data }: UpdateCompanyData) => {
      ...
      const extraFields = ['legal_name','domain','description','categories','founded_year','annual_revenue_range','funding_amount','business_model','country','timezone','connection_strength','icp_fit_score','pare_score','account_value_estimate','estimated_ltv','primary_use_case','decision_maker_role','priority_level','ai_summary','ai_tags','ai_pain_points','ai_opportunities','ai_risk_flags','custom_fields','company_status','postal_code','city','region','county','parish','cae_codes','cae_description','capital_social','legal_nature','founding_date','fax','about','activity_description','racius_url','avatar_url'];
