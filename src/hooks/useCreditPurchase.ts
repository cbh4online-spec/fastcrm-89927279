...
export function useCreditPurchase() {
  ...
  // Purchase mutation — abre directamente o Stripe Checkout
  const purchaseCredits = useMutation({
    mutationFn: async (packageId: string) => {
      ...
      if (error) throw error;
      if (data?.error || data?.fallback) throw new Error(data?.error || "Não foi possível iniciar a compra");
      if (!data?.url) throw new Error("Sessão de pagamento inválida");
      return data as { url: string; sessionId: string };
    },
    onSuccess: (data) => {
      if (data?.url) {
        // Redirecciona na mesma aba para evitar bloqueio de pop-up
