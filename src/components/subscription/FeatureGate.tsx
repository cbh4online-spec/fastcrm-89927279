...
import { useSubscription, PlanLimits, PLAN_INFO, SubscriptionPlan } from "@/contexts/SubscriptionContext";
...
export function UpgradePrompt({ 
  ...
  const suggestedPlan: SubscriptionPlan = currentPlan === "starter" ? "growth" : "scale";
...
export function LockedOverlay({ feature, children }: LockedOverlayProps) {
  ...
  const suggestedPlan: SubscriptionPlan = plan === "starter" ? "growth" : "scale";
    ...
    <div className="relative">
      ...
      <div className="absolute inset-0 flex items-center justify-center bg-background/60 backdrop-blur-[1px] rounded-lg">
        <div className="text-center p-4">
          ...
          <p className="text-sm font-medium text-foreground mb-2">Feature locked</p>
          <Button 
            size="sm" 
            onClick={() => createCheckout(suggestedPlan)}
            className="gap-1"
          >
            <Crown className="w-3 h-3" />
            Upgrade
...
export function PlanBadge({ plan, className }: PlanBadgeProps) {
  const planColors: Record<string, string> = {
    ...
    scale: "bg-gradient-to-r from-amber-500/10 to-orange-500/10 text-amber-600 border-amber-500/30",
    // Legacy
  ...
  return (
    <Badge 
      variant="outline" 
      className={cn("font-medium", planColors[plan] || planColors.starter, className)}
    >
      {(plan === "scale" || plan === "agency") && <Crown className="w-3 h-3 mr-1" />}
