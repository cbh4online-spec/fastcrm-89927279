...
import {
  History, Loader2, ShieldAlert, AlertCircle, PauseCircle, PlayCircle, Pencil,
  Globe2, Monitor,
} from "lucide-react";
import { useWorkspaceAuditLog, type WorkspaceAuditEntry } from "@/hooks/useWorkspaceAuditLog";
...
const ACTION_META: Record<string, ActionMeta> = {
  suspend_workspace:    { label: "Suspendeu o workspace", icon: PauseCircle, tone: "warning" },
  reactivate_workspace: { label: "Reativou o workspace",  icon: PlayCircle,  tone: "success" },
  update_metadata:      { label: "Atualizou metadados",   icon: Pencil,      tone: "info"    },
  // legado (mutations diretas pré-2F.1)
  "workspace.suspend":          { label: "Suspendeu (legado)", icon: PauseCircle, tone: "warning" },
  "workspace.reactivate":       { label: "Reativou (legado)",  icon: PlayCircle,  tone: "success" },
  "workspace.update_metadata":  { label: "Metadados (legado)", icon: Pencil,      tone: "info"    },
};
...
export function WorkspaceAuditTimeline({ workspaceId }: { workspaceId: string }) {
  const { data, isLoading, isError, error } = useWorkspaceAuditLog(workspaceId);
  ...
  if (entries.length === 0) {
    return (
      <EmptyState
        ...
        hint="As ações administrativas executadas neste workspace aparecerão aqui."
      />
    );
  }
...
function AuditItem({ entry, index }: { entry: WorkspaceAuditEntry; index: number }) {
