...
const comparisonIcons = [Table2, Box, Building];
const comparisonKeys = ["vsSpreadsheets", "vsTraditional", "vsEnterprise"];
...
export function LandingComparisonSection() {
  ...
  const comparisons = comparisonKeys.map((key, i) => ({
    icon: comparisonIcons[i],
    title: t(`comparison.${key}`).toUpperCase(),
    pain: t(`comparison.${key}P`),
    solution: t(`comparison.${key}S`),
  }));
