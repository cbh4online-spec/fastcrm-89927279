        ...
        <div className="grid md:grid-cols-3 gap-6">
          {comparisons.map((item, i) => (
            <motion.div
              ...
              className="rounded-2xl border border-[hsl(217,33%,17%)] bg-[hsl(222,47%,6%)] p-6 flex flex-col"
            >
              <div className="w-11 h-11 rounded-xl bg-primary/10 flex items-center justify-center mb-5">
                <item.icon className="h-5 w-5 text-primary" />
              </div>
              <h3 className="text-base font-bold mb-3">{item.title}</h3>
              <p className="text-sm text-[hsl(215,20%,65%)] leading-relaxed mb-4 line-through decoration-[hsl(0,84%,60%)]/40">
                {item.pain}
              </p>
              <p className="text-sm font-medium text-[hsl(210,40%,98%)] leading-relaxed mt-auto">
                {item.solution}
              </p>
            </motion.div>
          ))}
