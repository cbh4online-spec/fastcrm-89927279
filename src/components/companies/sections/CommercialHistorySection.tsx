...
export function CommercialHistorySection({ company, onFieldChange }: CommercialHistorySectionProps) {
  ...
  // Manual sales from company fields
  const manualSalesByYear: Record<number, number | null> = {
    2026: (company as any).sales_2026 ?? null,
    2025: (company as any).sales_2025 ?? null,
    2024: (company as any).sales_2024 ?? null,
    2023: (company as any).sales_2023 ?? null,
  };
  ...
  const lastPurchaseDate = useMemo(() => {
    const paidInvoices = companyInvoices
    ...
    return (company as any).last_purchase_date;
  }, [companyInvoices, countableInvoices, company]);

  // Calculate average ticket
  const averageTicket = useMemo(() => {
    if (invoiceCount === 0) return (company as any).average_ticket || 0;
    return calculatedTotalRevenue / invoiceCount;
  }, [calculatedTotalRevenue, invoiceCount, company]);
  ...
  const yearFieldMap: Record<number, string> = {
    2026: 'sales_2026',
    2025: 'sales_2025',
    2024: 'sales_2024',
    2023: 'sales_2023',
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <InlineEditableField
                      ...
                      onChange={(val) => fieldName && onFieldChange(fieldName as keyof Company, val)}
