...
import {
  useSmartAlerts, useDismissAlert, useActionAlert, ALERT_CONFIGS, SmartAlert,
...
export function DashboardSmartAlerts({ className, maxAlerts = 5 }: DashboardSmartAlertsProps) {
      ...
      <Card className={className}>
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center py-6 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">{t('noAlertsNeeded')}</p>
          </div>
        </CardContent>
      </Card>
    );
        ...
        <div className="flex items-center justify-between">
          ...
          {criticalAlerts.length > 0 && (
            <Badge variant="destructive" className="text-xs">
              {t('criticalCount', { count: criticalAlerts.length })}
            </Badge>
          )}
        </div>
      </CardHeader>
          ...
          <div className="divide-y px-4 pb-2">
            {sortedAlerts.map((alert) => {
              ...
              return (
                ...
                >
                  <div className={cn("w-8 h-8 rounded-full flex items-center justify-center text-lg flex-shrink-0", config.bgColor, config.borderColor, "border")}>
                    {config.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <Button variant="ghost" size="icon" className="h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" onClick={() => dismissAlert.mutate(alert.id)}>
                        <X className="w-3 h-3" />
                      </Button>
                    </div>
                    <div className="flex items-center justify-between mt-2">
                      ...
                      {alert.action_label && (
                        <Button variant="ghost" size="sm" className="h-6 text-xs gap-1 -mr-2" onClick={() => handleAction(alert)}>
                          {alert.action_label}
