...
function ItemTag({ isPro, isBeta }: { isPro?: boolean; isBeta?: boolean }) {
  if (isPro)
    return (
      <Badge variant="outline" className="ml-auto h-4 px-1.5 text-[10px] font-semibold border-amber-500/40 text-amber-500">
        Pro
      </Badge>
    );
  if (isBeta)
    return (
      <Badge variant="outline" className="ml-auto h-4 px-1.5 text-[10px] font-semibold border-blue-400/40 text-blue-400">
        Beta
      </Badge>
    );
  return null;
...
export function WatidySidebar({ open, onClose }: WatidySidebarProps) {
  ...
  // Detect mega-group from current route
  ...
  // Active mega selected by user (sticky), defaulting to route detection
  ...
  useEffect(() => {
    if (activeMegaFromRoute && activeMegaFromRoute !== activeMega) {
      setActiveMega(activeMegaFromRoute);
    }
  ...
  const isGroupOpen = useCallback(
    (key: string, items: RouteEntry[]) => {
      ...
      // open by default if has active or there's a search filter
  ...
  // Aggregated badge count per mega-group (for rail dot)
  ...
  // Layout: full sidebar width = rail (56) + panel (248) = 304
    ...
    <TooltipProvider delayDuration={0}>
      ...
      >
        ...
        <div className="flex flex-col items-center w-14 shrink-0 border-r border-sidebar-border py-2 gap-1">
          ...
          {/* Mega group icons */}
          <nav className="flex flex-col items-center gap-1 flex-1 overflow-y-auto scrollbar-thin w-full px-1.5" aria-label="Áreas">
            {megaGroups.map((mg) => {
              const Icon = mg.icon;
                ...
                <Tooltip key={mg.key}>
                  <TooltipTrigger asChild>
                    ...
                    >
                      ...
                      {count > 0 && (
                        <span className="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-sidebar-primary px-1 text-[9px] font-bold text-sidebar-primary-foreground">
                          {count > 99 ? "99+" : count}
                        </span>
                      )}
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="right">{mg.label}</TooltipContent>
                </Tooltip>
              );
            })}
          </nav>

          {/* Footer rail: collapse toggle + avatar */}
        ...
        {panelOpen && activeMegaData && (
          <div className="flex flex-col w-[248px] min-w-0">
            ...
            <div className="px-3 py-3 border-b border-sidebar-border flex items-center gap-2">
              <activeMegaData.icon className="w-4 h-4 text-sidebar-primary shrink-0" />
            ...
            {/* Sections */}
            <nav className="flex-1 overflow-y-auto scrollbar-thin px-2 py-2 space-y-1" aria-label="Subgrupos">
