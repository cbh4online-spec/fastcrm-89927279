...
const OpportunityRow = memo(function OpportunityRow({
  ...
  return (
    ...
    >
      ...
      <TableCell className="text-center">
        <DealHealthBadge intelligence={intelligence} />
      </TableCell>
...
export function OpportunityTableView({
  ...
  const getTemperatureColor = useCallback((temp: string | null) => {
    switch (temp) {
  ...
  const useVirtual = sortedOpportunities.length > 50;
    ...
    if (!useVirtual) {
      return sortedOpportunities.map((opp) => (
        <OpportunityRow
          ...
          score={scoresMap?.get(opp.id)}
          intelligence={healthMap?.get(opp.id)}
          locale={locale}
          t={t}
    ...
    return (
      <>
        ...
        {items.map((virtualRow) => {
          const opp = sortedOpportunities[virtualRow.index];
          return (
            <OpportunityRow
              key={opp.id}
              opp={opp}
  ...
  return (
    <div className="border rounded-lg flex flex-col">
      <div
        ...
        className={cn(
          "overflow-auto",
          useVirtual && "max-h-[calc(100vh-280px)]"
        )}
      >
        <Table>
          <TableHeader className={cn(useVirtual && "sticky top-0 z-10 bg-card")}>
            <TableRow>
