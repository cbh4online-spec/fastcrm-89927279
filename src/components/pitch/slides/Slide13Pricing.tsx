  ...
  return (
    <SlideShell variant="light">
      <div className="absolute inset-0" style={{ padding: '100px 80px' }}>
        <SlideHeader eyebrow="Investimento" title="Planos e investimento" subtitle={`Proposta dimensionada para ${company} — sem custos de instalação no primeiro ano.`} />
        <div className={`grid gap-6 mt-8 ${plans.length >= 4 ? 'grid-cols-4' : 'grid-cols-3'}`}>
          {plans.map((p) => (
            <div
              key={p.name}
              className={`rounded-2xl p-7 border-2 ${p.highlight ? 'border-[#22D3EE] bg-gradient-to-br from-[#CFFAFE] to-white shadow-xl scale-[1.02]' : 'border-[#E2E8F0] bg-white'}`}
            >
              {p.highlight && (
