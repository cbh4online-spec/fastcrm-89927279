...
import { FlowStep, STEP_TYPE_CONFIG, FlowStepType, isAIStepType } from '@/types/conversational-flows';
...
function FlowStepNodeComponent({ data, selected }: FlowStepNodeProps) {
  ...
  const isAI = isAIStepType(step.stepType);
    ...
    >
      ...
      <div className="px-3 py-2 text-xs">
        {step.stepType === 'message' && step.messageContent && (
          <p className="text-muted-foreground line-clamp-2">"{step.messageContent}"</p>
        )}
        ...
        {step.stepType === 'action' && step.actionType && (
          <p className="text-muted-foreground">Ação: {step.actionType}</p>
        )}
        {step.stepType === 'goal' && step.goalName && (
          <p className="text-muted-foreground">🎯 {step.goalName}</p>
        )}
        {step.stepType === 'handoff' && (
          <p className="text-muted-foreground">Transferir para agente humano</p>
        )}
        {isAI && (
          <div className="space-y-1">
            {step.aiInputSource && (
              <p className="text-muted-foreground">
                📥 {step.aiInputSource === 'current_lead' ? 'Lead' : step.aiInputSource === 'current_company' ? 'Empresa' : step.aiInputSource === 'conversation_history' ? 'Conversa' : step.aiInputSource}
              </p>
            )}
            {step.aiModel && (
              <p className="text-muted-foreground text-[10px]">
                🤖 {step.aiModel.split('/').pop()}
              </p>
            )}
            {step.aiOutputVariable && (
              <p className="text-muted-foreground text-[10px]">
                📤 → {step.aiOutputVariable}
              </p>
            )}
