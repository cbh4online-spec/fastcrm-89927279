...
export function LeadsKanbanView({ leads, isLoading }: LeadsKanbanViewProps) {
  ...
  const groupedLeads = useMemo(() => {
    ...
    for (const lead of leads) {
      ...
      if (groups[group]) groups[group].push(lead);
        ...
        groups[columns[columns.length - 1].key]?.push(lead);
      }
    }
    return groups;
  ...
  const handleDrop = useCallback(async (e: DragEvent, targetColumn: string) => {
    e.preventDefault();
    ...
    const value = targetColumn === "unknown" ? null : targetColumn;
  ...
  if (isLoading) {
    return (
      <div className="flex gap-4 p-4 h-full">
        {[1, 2, 3].map(i => (
          <div key={i} className="flex-1 min-w-[280px] rounded-lg border bg-muted/30 animate-pulse h-[400px]" />
        ))}
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full mt-4">
      ...
      <ScrollArea className="flex-1 px-4 pb-4">
        <div className="flex gap-4 min-h-[500px]">
          {columns.map((col) => {
            const items = groupedLeads[col.key] || [];
            return (
              <div
                key={col.key}
                className={cn(
                  ...
                  col.color,
                  dragOverColumn === col.key && "ring-2 ring-primary/50"
                )}
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-2.5">
                          <GripVertical className="h-4 w-4 text-muted-foreground/40 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity cursor-grab shrink-0" />
                        ...
                        {(lead as any).tags?.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-2 ml-[42px]">
                            {((lead as any).tags as string[]).slice(0, 3).map(tag => (
                              <Badge key={tag} variant="secondary" className="text-[10px] px-1.5 py-0 h-4">
                                {tag}
                              </Badge>
                            ))}
                            {(lead as any).tags.length > 3 && (
                              <span className="text-[10px] text-muted-foreground">+{(lead as any).tags.length - 3}</span>
                            )}
                          </div>
                        )}
                      </div>
