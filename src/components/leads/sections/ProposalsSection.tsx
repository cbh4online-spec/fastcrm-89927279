...
function useLeadProposals(leadId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // First get opportunities for this lead
      const { data: opportunities, error: oppError } = await workspaceClient
        .from("opportunities")
        .select("id")
        .eq("lead_id", leadId);
      ...
      // Then get proposals for those opportunities
...
function useDeleteProposal() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["proposals", "lead", data.leadId] });
...
export function ProposalsSection({ leadId, leadName }: ProposalsSectionProps) {
  ...
  const handleDelete = async (proposal: ProposalWithOpportunity) => {
    try {
      await deleteProposal.mutateAsync({ id: proposal.id, leadId });
  ...
  // Calculate summary
  const totalValue =
    proposals?.reduce((sum, p) => sum + (p.price || 0), 0) || 0;
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-sm mb-4">
              Este lead ainda não tem propostas. Crie uma oportunidade primeiro e depois adicione uma proposta.
            </p>
          </CardContent>
        </Card>
      ) : (
