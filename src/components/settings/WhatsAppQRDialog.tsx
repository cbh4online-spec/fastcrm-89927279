...
type Status = "idle" | "loading" | "qr_ready" | "scanning" | "connected" | "error";
...
export function WhatsAppQRDialog({ open, onOpenChange }: WhatsAppQRDialogProps) {
  ...
  useEffect(() => {
    if (status !== "qr_ready" || !instanceName || !currentWorkspace?.id) return;
        ...
        <div className="flex flex-col items-center gap-4 py-4">
          ...
          {status === "error" && (
            <div className="flex flex-col items-center gap-3 py-8">
              ...
              <p className="text-sm text-destructive">{errorMsg || "Erro ao gerar QR Code"}</p>
