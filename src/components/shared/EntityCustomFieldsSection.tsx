...
export function EntityCustomFieldsSection({ entityType, entityId }: EntityCustomFieldsSectionProps) {
  const { data: customFields = [], isLoading: isLoadingFields } = useCustomFields(entityType);
  const { data: fieldValues = [], isLoading: isLoadingValues } = useCustomFieldValues(entityId);
  const setFieldValue = useSetCustomFieldValue();

  const isLoading = isLoadingFields || isLoadingValues;

  const entityLabel = entityType === "contact" ? "contactos" : entityType === "company" ? "empresas" : "leads";
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Ainda não existem campos personalizados para {entityLabel}.
            </p>
