  ...
  switch (columnId) {
    case "name": {
      ...
      return (
        ...
        >
          {(product.images?.[0] || product.product_images?.[0]?.url) ? (
            <img src={product.images?.[0] || product.product_images?.[0]?.url} alt={product.name} className="w-8 h-8 rounded object-cover flex-shrink-0" />
          ) : (
