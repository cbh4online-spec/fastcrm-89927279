...
export default function SecuritySystemDetailPage() {
  ...
  const handleAddZone = async () => {
    const { error } = await supabase.from("security_system_zones").insert({ system_id: id, ...zoneForm } as any);
  ...
  const handleAddDevice = async () => {
    const payload: any = {
      system_id: id, device_type: deviceForm.device_type, brand: deviceForm.brand,
      model: deviceForm.model, serial_number: deviceForm.serial_number || null,
      quantity: Number(deviceForm.quantity) || 1, location_description: deviceForm.location_description || null,
    };
      ...
      <Dialog open={addZoneOpen} onOpenChange={setAddZoneOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <div><Label>Nome da Zona</Label><Input value={zoneForm.zone_name} onChange={(e) => setZoneForm(p => ({ ...p, zone_name: e.target.value }))} placeholder="Ex: Entrada principal" /></div>
            <div>
              <Label>Tipo</Label>
            ...
            <div><Label>Notas</Label><Input value={zoneForm.notes} onChange={(e) => setZoneForm(p => ({ ...p, notes: e.target.value }))} /></div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><Label>{t("quantity")}</Label><Input type="number" value={deviceForm.quantity} onChange={(e) => setDeviceForm(p => ({ ...p, quantity: e.target.value }))} min="1" /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label>{t("brand")}</Label><Input value={deviceForm.brand} onChange={(e) => setDeviceForm(p => ({ ...p, brand: e.target.value }))} /></div>
              <div><Label>{t("model")}</Label><Input value={deviceForm.model} onChange={(e) => setDeviceForm(p => ({ ...p, model: e.target.value }))} /></div>
            </div>
            <div><Label>{t("serialNumber")}</Label><Input value={deviceForm.serial_number} onChange={(e) => setDeviceForm(p => ({ ...p, serial_number: e.target.value }))} /></div>
            {zones.length > 0 && (
              <div>
                <Label>Zona</Label>
                <Select value={deviceForm.zone_id} onValueChange={(v) => setDeviceForm(p => ({ ...p, zone_id: v }))}>
                  ...
                  <SelectContent>
                    {zones.map((z: any) => <SelectItem key={z.id} value={z.id}>{z.zone_name}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            )}
            <div><Label>Localização</Label><Input value={deviceForm.location_description} onChange={(e) => setDeviceForm(p => ({ ...p, location_description: e.target.value }))} placeholder="Ex: Hall de entrada, teto" /></div>
            <div className="flex justify-end gap-2">
