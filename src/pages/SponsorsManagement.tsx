...
import {
  ...
  Award, Users, TrendingUp, Crown, Medal, Star, ExternalLink,
} from "lucide-react";
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold">Sponsors & Parceiros</h1>
        </div>
        <p className="text-muted-foreground">Gerir parceiros, candidaturas e métricas de sponsoring do workspace.</p>
      </div>
      ...
      <Tabs defaultValue="sponsors" className="space-y-4">
        <TabsList>
          <TabsTrigger value="sponsors">Parceiros ({sponsors.length})</TabsTrigger>
        ...
        <TabsContent value="sponsors" className="space-y-4">
          <div className="flex justify-between items-center flex-wrap gap-3">
            ...
            <Button onClick={openCreateSponsor}><Plus className="h-4 w-4 mr-1" /> Adicionar Parceiro</Button>
          </div>

          {filteredSponsors.length === 0 ? (
            <Card><CardContent className="py-12 text-center text-muted-foreground">
              {tierFilter === "all" ? "Nenhum parceiro registado. Adicione o primeiro!" : `Nenhum parceiro ${tierFilter} encontrado.`}
            </CardContent></Card>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filteredSponsors.map((s) => (
                <Card key={s.id} className={`relative group ${!s.is_active ? "opacity-60" : ""}`}>
                  <CardContent className="pt-6 space-y-3">
                    {/* Preview card */}
                    <div className="flex items-center gap-3">
                      {s.logo_url ? (
                        <img src={s.logo_url} alt={s.name} className="h-12 w-12 rounded-lg object-cover border" />
                      ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full capitalize ${TIER_COLORS[s.tier]}`}>
                        ...
                        {s.website_url && (
                          <a href={s.website_url} target="_blank" rel="noopener noreferrer"
                            ...
                            <ExternalLink className="h-3 w-3 shrink-0" />{s.website_url}
                          </a>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t">
                      ...
                      <div className="flex gap-1">
                        <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => openEditSponsor(s)}>
                          <Pencil className="h-3.5 w-3.5" />
                        </Button>
                        <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" onClick={() => setDeleteConfirm(s.id)}>
      ...
      <Dialog open={sponsorDialog.open} onOpenChange={(o) => setSponsorDialog({ open: o, editing: sponsorDialog.editing })}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{sponsorDialog.editing ? "Editar Parceiro" : "Novo Parceiro"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div><Label>Nome *</Label><Input value={sponsorForm.name} onChange={(e) => setSponsorForm((f) => ({ ...f, name: e.target.value }))} placeholder="Nome da empresa" /></div>
            <div><Label>URL do Logótipo</Label><Input value={sponsorForm.logo_url} onChange={(e) => setSponsorForm((f) => ({ ...f, logo_url: e.target.value }))} placeholder="https://..." /></div>
            {sponsorForm.logo_url && (
              <div className="flex items-center gap-3 p-3 rounded-lg border bg-muted/30">
                <img src={sponsorForm.logo_url} alt="Preview" className="h-10 w-10 rounded-md object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                <span className="text-xs text-muted-foreground">Preview do logótipo</span>
              </div>
            )}
            <div><Label>Website</Label><Input value={sponsorForm.website_url} onChange={(e) => setSponsorForm((f) => ({ ...f, website_url: e.target.value }))} placeholder="https://..." /></div>
            <div><Label>Descrição</Label><Textarea value={sponsorForm.description} onChange={(e) => setSponsorForm((f) => ({ ...f, description: e.target.value }))} placeholder="Breve descrição do parceiro..." rows={3} /></div>
            <div>
              <Label>Tier</Label>
              <Select value={sponsorForm.tier} onValueChange={(v) => setSponsorForm((f) => ({ ...f, tier: v as "gold" | "silver" | "bronze" }))}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="gold">🥇 Gold — {TIER_PRICES.gold}€/mês</SelectItem>
                  <SelectItem value="silver">🥈 Silver — {TIER_PRICES.silver}€/mês</SelectItem>
                  <SelectItem value="bronze">🥉 Bronze — {TIER_PRICES.bronze}€/mês</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div><Label>Ordem de exibição</Label><Input type="number" value={sponsorForm.sort_order} onChange={(e) => setSponsorForm((f) => ({ ...f, sort_order: Number(e.target.value) }))} /></div>
            ...
            {sponsorForm.name && (
              <div className="border rounded-lg p-4 bg-muted/20">
                ...
                <div className="flex items-center gap-3">
                  {sponsorForm.logo_url ? (
                    <img src={sponsorForm.logo_url} alt="" className="h-10 w-10 rounded-md object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                  ) : (
                    <div className="h-10 w-10 rounded-md bg-muted flex items-center justify-center font-bold">{sponsorForm.name.charAt(0)}</div>
                  )}
                  <div>
                    <p className="font-semibold text-sm">{sponsorForm.name}</p>
                    {sponsorForm.description && <p className="text-xs text-muted-foreground line-clamp-1">{sponsorForm.description}</p>}
                  </div>
                  <span className={`ml-auto inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full capitalize ${TIER_COLORS[sponsorForm.tier]}`}>
                    {TIER_ICONS[sponsorForm.tier]} {sponsorForm.tier}
                  </span>
                </div>
              </div>
            )}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveSponsor} disabled={!sponsorForm.name.trim() || createSponsor.isPending || updateSponsor.isPending}>
              {sponsorDialog.editing ? "Guardar" : "Criar Parceiro"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deleteConfirm} onOpenChange={(o) => !o && setDeleteConfirm(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Esta ação é irreversível. O parceiro será removido permanentemente.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
