...
export default function CheckoutPage() {
  ...
  async function loadFunnel() {
    try {
      const { data: funnelData } = await supabase.from("checkout_funnels").select("*").eq("slug", funnelSlug).eq("is_active", true).single();
      if (!funnelData) { toast.error("Checkout não encontrado"); return; }
      setFunnel(funnelData);

      const { data: bumpData } = await supabase.from("checkout_order_bumps")
