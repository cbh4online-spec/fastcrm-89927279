...
import { Plus, Search, MoreHorizontal, Eye, Trash2, GraduationCap, Users, Calendar } from "lucide-react";
...
export default function SJCohorts() {
  ...
  const statuses: (CohortStatus | "all")[] = ["all", "planned", "open", "running", "finished"];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Gerencie turmas e coortes</p>
        </div>
        <Button onClick={() => setCreateDialogOpen(true)} className="gap-2"><Plus className="h-4 w-4" />Nova Turma</Button>
      </div>
        ...
        <div className="flex gap-2">
          {statuses.map((status) => (
            <Button key={status} variant={statusFilter === status ? "default" : "outline"} size="sm" onClick={() => setStatusFilter(status)}>
              {status === "all" ? "Todas" : COHORT_STATUS_CONFIG[status].label}
            </Button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          <div className="col-span-full text-center py-8 text-muted-foreground">A carregar...</div>
        ) : filteredCohorts.length === 0 ? (
          <div className="col-span-full text-center py-8 text-muted-foreground">Nenhuma turma encontrada</div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {cohort.course && <p className="text-sm text-muted-foreground">{cohort.course.name}</p>}
                    </div>
                  </div>
                  <DropdownMenu>
      ...
      <CreateCohortDialog open={createDialogOpen} onOpenChange={setCreateDialogOpen} />
    </div>
  );
}
