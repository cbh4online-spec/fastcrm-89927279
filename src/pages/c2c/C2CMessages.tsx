...
export default function C2CMessages() {
  ...
  const handleSend = useCallback((content: string, messageType?: string, metadata?: Record<string, unknown>) => {
    if (!selected) return;
    sendMessage.mutate({ listingId: selected.listing_id, receiverId: selected.other_user_id, content, messageType, metadata });
  }, [selected, sendMessage]);

  const handleMakeOffer = useCallback((price?: number) => {
    if (price !== undefined && selected) {
      sendOffer.mutate({ listingId: selected.listing_id, sellerId: selected.listing_seller_id, offerPrice: price });
    } else {
  ...
  const handleSubmitOffer = useCallback(() => {
    ...
    sendOffer.mutate({ listingId: selected.listing_id, sellerId: selected.listing_seller_id, offerPrice: parseFloat(offerAmount) });
    setOfferDialogOpen(false);
  }, [selected, offerAmount, sendOffer]);

  const handleRespondOffer = useCallback((offerId: string, action: "accepted" | "rejected") => {
    if (!selected) return;
    respondOffer.mutate({ offerId, action, buyerId: selected.other_user_id, listingId: selected.listing_id });
  }, [selected, respondOffer]);

  const handleUpdateStatus = useCallback((status: "reserved" | "sold") => {
    if (!selected) return;
    updateStatus.mutate({ listingId: selected.listing_id, newStatus: status, otherUserId: selected.other_user_id });
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-4 max-w-7xl">
        ...
        <div className="hidden lg:grid lg:grid-cols-[320px_1fr_360px] gap-3 h-[calc(100vh-120px)]">
          <ConversationList conversations={conversations} isLoading={isLoading} selectedKey={selectedKey} onSelect={handleSelect} />
          <ChatThread conversation={selected} messages={threadMessages} offers={offers} onSend={handleSend} onMakeOffer={() => handleMakeOffer()} onRespondOffer={handleRespondOffer} onUpdateStatus={handleUpdateStatus} isSending={sendMessage.isPending} />
          <DealPanel conversation={selected} offers={offers} onMakeOffer={handleMakeOffer} />
        </div>
        ...
        <div className="lg:hidden h-[calc(100vh-120px)]">
          <Tabs value={mobileTab} onValueChange={setMobileTab} className="h-full flex flex-col">
            <TabsList className="grid grid-cols-3 mb-3">
              ...
              <TabsTrigger value="deal">{t('product')}</TabsTrigger>
            </TabsList>
      ...
      <Dialog open={offerDialogOpen} onOpenChange={setOfferDialogOpen}>
        <DialogContent className="sm:max-w-[380px]">
          <DialogHeader>
            <DialogTitle>{t('makeProposal')}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            <div>
              <p className="text-xs text-muted-foreground mb-1">{t('advertisedPrice')}</p>
              <p className="text-lg font-bold text-[hsl(var(--gold))]">{selected?.listing_price.toFixed(2)} €</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground mb-1">{t('yourProposal')}</p>
              <Input type="number" step="0.01" value={offerAmount} onChange={e => setOfferAmount(e.target.value)} placeholder="0.00" className="text-lg font-semibold" />
            </div>
