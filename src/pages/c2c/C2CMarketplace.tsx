...
import { useC2CRealtimeNotifications } from "@/hooks/useC2CRealtimeNotifications";
...
function SectionCarousel({ title, icon, listings, favoriteIds, onToggleFavorite, onNavigate, seeMoreHref, seeMoreLabel }: {
  title: string;
      ...
      <div className="flex items-center justify-between">
        ...
        {seeMoreHref && (
          <Button variant="ghost" size="sm" className="gap-1 text-primary" onClick={seeMoreHref}>
            {seeMoreLabel || "ver mais"} <ChevronRight className="h-4 w-4" />
          </Button>
        )}
      </div>
      <div className="relative">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 max-w-xl mx-auto">
              ...
              >
                ...
                {filters.search || t('searchPlaceholder')}
              </button>
            </div>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {user && (
                <>
                  <Button variant="ghost" size="icon" className="relative" onClick={() => navigate("/dashboard/c2c/favorites")}>
                    <Heart className="h-5 w-5" />
                  </Button>
                  <Button variant="ghost" size="icon" onClick={() => navigate("/dashboard/c2c/messages")}>
                    <MessageCircle className="h-5 w-5" />
                  </Button>
                  <Button variant="ghost" size="icon" className="relative" onClick={() => navigate("/dashboard/c2c/notifications")}>
                  ...
                  <Button size="sm" className="gap-1 rounded-full hidden sm:flex" onClick={() => navigate("/dashboard/c2c/create")}>
                    <Plus className="h-4 w-4" />
                    {t('sell')}
                  </Button>
                  <Button size="icon" className="sm:hidden rounded-full" onClick={() => navigate("/dashboard/c2c/create")}>
                    <Plus className="h-4 w-4" />
                  </Button>
                </>
              )}
      ...
      <main className="container mx-auto px-4 py-6 space-y-8">
        ...
        {hasActiveFilters ? (
          <div className="space-y-6">
            <ListingFilters filters={filters} onFiltersChange={setFilters} categories={categories} />
            ...
            {isLoading ? (
              ...
              <>
                <p className="text-sm text-muted-foreground">
                  {listings.length} {listings.length !== 1 ? t('resultsPlural') : t('results')}
                </p>
                <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                  {listings.map((listing) => (
                    <ListingCard
          ...
          <div className="space-y-10">
            ...
            {listings.length === 0 && !isLoading && (
              <div className="text-center py-20">
                ...
                <h3 className="text-xl font-semibold mb-2">{t('emptyMarketplace')}</h3>
                <p className="text-muted-foreground mb-6">{t('beFirstToPublish')}</p>
                {user && (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-8 border-t">
          <div className="flex flex-col items-center text-center gap-2 p-4">
            ...
            <span className="text-sm font-medium">{t('trustSafePurchase')}</span>
            <span className="text-[11px] text-muted-foreground">{t('trustSafePurchaseDesc')}</span>
          </div>
          <div className="flex flex-col items-center text-center gap-2 p-4">
            <Truck className="h-8 w-8 text-primary" />
            <span className="text-sm font-medium">{t('trustNationalShipping')}</span>
            <span className="text-[11px] text-muted-foreground">{t('trustNationalShippingDesc')}</span>
          </div>
          <div className="flex flex-col items-center text-center gap-2 p-4">
            <Award className="h-8 w-8 text-primary" />
            <span className="text-sm font-medium">{t('trustVerifiedSellers')}</span>
            <span className="text-[11px] text-muted-foreground">{t('trustVerifiedSellersDesc')}</span>
          </div>
          <div className="flex flex-col items-center text-center gap-2 p-4">
            <MessageCircle className="h-8 w-8 text-primary" />
            <span className="text-sm font-medium">{t('trustDirectChat')}</span>
            <span className="text-[11px] text-muted-foreground">{t('trustDirectChatDesc')}</span>
          </div>
        </div>
      </main>
