...
BEGIN
  ...
  RETURN QUERY
  WITH movements AS (
    SELECT im.product_id, im.created_at,
    ...
    FROM inventory_movements im WHERE im.workspace_id = _workspace_id
    UNION ALL
    SELECT psm.product_id, psm.created_at,
      CASE WHEN psm.movement_type IN ('in','purchase','adjustment_in','return') THEN psm.quantity ELSE 0 END,
      CASE WHEN psm.movement_type IN ('out','sale','adjustment_out','transfer') THEN psm.quantity ELSE 0 END,
      COALESCE(psm.unit_cost, 0)
    FROM product_stock_movements psm WHERE psm.workspace_id = _workspace_id
  ),
  fifo_layers AS (
  ...
  remaining_layers AS (
    ...
    FROM fifo_layers fl WHERE fl.qty_in > 0
  ),
  ...
  computed AS (
    SELECT p.id, p.name, p.sku, p.category,
      COALESCE(p.low_stock_threshold, 0)::integer AS low_threshold,
      ...
      COALESCE(p.target_margin_mode, 'percent')::text AS target_mode
    FROM products p
  ...
  SELECT
    ...
    c.stock_eff, c.low_threshold,
    c.cost_eff, c.op_cost_unit,
    (c.cost_eff + c.op_cost_unit)::numeric AS total_unit_cost,
    (c.stock_eff * (c.cost_eff + c.op_cost_unit))::numeric AS total_cost_value,
    c.base_price_curr AS unit_sale_price,
    CASE
      WHEN c.target_pct <= 0 THEN NULL
      WHEN c.target_mode = 'markup'
        THEN ((c.cost_eff + c.op_cost_unit) * (1 + c.target_pct / 100.0))::numeric
      WHEN c.target_pct < 100
        THEN ((c.cost_eff + c.op_cost_unit) / (1 - c.target_pct / 100.0))::numeric
      ELSE NULL
    END AS suggested_base_price,
    (c.stock_eff * c.base_price_curr)::numeric AS total_sale_value,
    (c.stock_eff * (c.base_price_curr - (c.cost_eff + c.op_cost_unit)))::numeric AS latent_margin,
    CASE WHEN c.base_price_curr > 0
      THEN ((c.base_price_curr - (c.cost_eff + c.op_cost_unit)) / c.base_price_curr * 100)::numeric
      ELSE 0 END AS latent_margin_pct,
    CASE WHEN (c.cost_eff + c.op_cost_unit) > 0
      THEN ((c.base_price_curr - (c.cost_eff + c.op_cost_unit)) / (c.cost_eff + c.op_cost_unit) * 100)::numeric
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(v.current_stock), 0)::numeric AS total_units,
    COALESCE(SUM(v.total_cost_value), 0)::numeric AS total_cost_value,
    COALESCE(SUM(v.current_stock * v.operational_cost_unit), 0)::numeric AS total_operational_cost_value,
    COALESCE(SUM(v.total_sale_value), 0)::numeric AS total_sale_value,
    COALESCE(SUM(v.latent_margin), 0)::numeric AS total_latent_margin,
    COALESCE(AVG(NULLIF(v.latent_margin_pct, 0)), 0)::numeric AS avg_margin_pct,
    COUNT(*) FILTER (WHERE v.current_stock <= 0)::bigint AS zero_stock_count,
    COUNT(*) FILTER (WHERE v.latent_margin < 0)::bigint AS negative_margin_count
  FROM v;
END;
$function$;