...
BEGIN
  -- Authorization: must be member of workspace
  IF NOT EXISTS (
  ...
  kpis AS (
    SELECT
      ...
      COALESCE(SUM(amount_paid),0)::numeric AS total_received,
      COALESCE(SUM(GREATEST(total - COALESCE(amount_paid,0), 0)) FILTER (WHERE due_date IS NOT NULL AND due_date < v_today AND (total - COALESCE(amount_paid,0)) > 0),0)::numeric AS overdue,
  ...
  top_clients AS (
    SELECT
      ...
      SUM(COALESCE(f.amount_paid,0))::numeric AS received,
      SUM(f.total - COALESCE(f.amount_paid,0))::numeric AS due,
      COUNT(*)::int AS cnt
    FROM filtered_inv f
  ...
  aging AS (
    ...
    FROM (
      SELECT due_date, (total - COALESCE(amount_paid,0)) AS remaining
      FROM filtered_inv
      WHERE (total - COALESCE(amount_paid,0)) > 0
    ) s
  ),
  owners AS (
