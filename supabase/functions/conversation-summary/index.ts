...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (workspaceId) {
      const gate = await aiGate(workspaceId, 'light', 'conversation-summary');
      if (!gate.allowed) {
    ...
    if (workspaceId) {
      try {
        ...
        logAIUsage({
          ...
          entity_type: conversationId ? 'conversation' : undefined,
          entity_id: conversationId ?? undefined,
        });
      } catch (e) {
        console.warn('[CONV-SUMMARY] logAIUsage failed:', e);
      }
    }
